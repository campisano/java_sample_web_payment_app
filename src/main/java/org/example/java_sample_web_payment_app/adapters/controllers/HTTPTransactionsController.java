...
public class HTTPTransactionsController {
    private static final Logger LOGGER = LoggerFactory.getLogger(
            HTTPTransactionsController.class);

    private CreateTransactionUsecasePort createTransactionUsecase;

    public HTTPTransactionsController(CreateTransactionUsecasePort
    ...
    public ResponseEntity<?> post(HttpServletRequest request,
                                  @RequestBody Optional<HTTPTransactionsPostRequest> body) {
        ...
        try {
            createTransactionUsecase.execute(body.get().accountId,
        ...
        } catch(AccountIdNotExistsException | OperationTypeIdNotExistsException
                    exception) {
            LOGGER.error("exception, message={}", exception.getMessage());
